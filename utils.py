...
processed_data = Path(__file__).resolve().parent / "data"
...
def t0_wc_data(path=os.path.join(processed_data, 'exp_whole_comm/df_b_reps.csv')):
    whole_comm_df = pd.read_csv(path, index_col=0)
    t0 = whole_comm_df[whole_comm_df.index == 0].drop(['rep'], axis=1)
    t0.index = ['crm_t0']
    return t0

def deviation_factor(t0=t0_wc_data()):
    theoretical_abundance = 1 / len(t0.T)
    deviation_factor = t0 / theoretical_abundance
    deviation_factor = deviation_factor.replace(0, 1)
    deviation_factor.index = ['deviation_factor']
    return deviation_factor

def cn_16s(path=os.path.join(processed_data, '16s_cn.csv')):
    cp_num = pd.read_csv(path)
    cp_num_df = cp_num.pivot_table(columns='sp_number', values='16s_copy_number')
    cp_num_df.columns = cp_num_df.columns.astype(str)
    return cp_num_df

def od_to_cfu(path=os.path.join(processed_data, 'od_cfu_counts.csv')):
    "Import OD-to-CFU Data (generated by Kendall)"
    name_to_sp = {v: k for k, v in sps_to_name.items()}
    od_cfu = pd.read_csv(path)
    od_cfu["species"] = od_cfu["Microorganism"].str.extract(r"^(\S+)\s+sp\.")
    means = od_cfu.groupby("species")["Average CFU ml^-1"].mean()
    od_cfu = means.reindex(sps_names, fill_value=1e9)
    od_cfu.index = [name_to_sp[name] for name in od_cfu.index]
    od_cfu_df = pd.DataFrame(od_cfu).T
    return od_cfu_df
